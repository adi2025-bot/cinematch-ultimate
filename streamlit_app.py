...
<style>
    ...
    /* ============================================ */
    /* MOBILE TOUCH OPTIMIZATION - ELIMINATE DELAY  */
    /* ============================================ */
    /* Remove 300ms tap delay on ALL interactive elements */
    a, button, input, select, textarea, 
    .movie-card, .nav-item, .share-icon, .cast-img,
    [role="button"], .stButton > button,
    ...
    @media (max-width: 768px) {
        ...
        .stat-pill { 
            ...
            /* Remove backdrop-filter on mobile pills */
            backdrop-filter: none;
            -webkit-backdrop-filter: none;
        }
        ...
        .share-icon { width: 44px; height: 44px; min-width: 44px; min-height: 44px; }
    ...
    @media (max-width: 480px) {
        ...
        .movie-card { 
            border-radius: 10px; 
            margin-bottom: 12px;
            /* Remove all box-shadows on mobile for perf */
            box-shadow: 0 2px 8px rgba(0,0,0,0.3);
        }
        .movie-card::before {
            display: none; /* Remove overlay effect on mobile */
        }
        .card-content { padding: 10px; }
        .card-title { font-size: 0.85rem; }
        ...
        /* Button adjustments for mobile - bigger touch targets */
        .stButton>button {
            padding: 12px 16px;
            font-size: 0.9rem;
            border-radius: 8px;
        ...
        /* Disable shimmer animation on mobile search */
        .stSelectbox > div > div::before {
            animation: none !important;
            display: none;
        }
    ...
    @media (hover: none) and (pointer: coarse) {
        /* Remove ALL hover transforms - they cause ghost effects on touch */
        .movie-card:hover { 
            transform: none !important; 
            box-shadow: 0 2px 8px rgba(0,0,0,0.3) !important;
            border-color: rgba(0, 212, 255, 0.2) !important;
        }
        .movie-card:hover::before { opacity: 0 !important; }
        .share-icon:hover { transform: none !important; }
        .cast-img:hover { transform: none !important; border-color: rgba(139, 92, 246, 0.2) !important; }
        .stSelectbox > div > div:hover { transform: none !important; }
        .stButton>button:hover { transform: none !important; }
        
        /* INSTANT touch press feedback - :active fires immediately with touch-action:manipulation */
        .movie-card:active { 
        ...
        /* Disable all expensive CSS animations on touch devices */
        .stColumn > div > div,
        .hero-container,
        .stMarkdown h3 {
            animation: none !important;
        ...
        /* Disable confetti/heart animations on low-power devices */
        .floating-heart, .confetti, .sparkle-effect {
            animation-duration: 0.8s !important; /* Shorter on mobile */
        }
        
        /* Simplify gradient backgrounds for GPU perf */
        .movie-card {
            background: #1a1a2e;
        }
        
        /* Remove all backdrop-filter instances */
        .nav-header,
        .info-box,
        .login-container,
        .stat-pill,
    ...
    @media (max-width: 768px) {
        .mobile-nav {
            ...
            left: 0;
            right: 0;
            /* Solid background instead of expensive backdrop-filter */
            background: #0d0b1e;
            display: flex;
...
if not st.session_state.logged_in:
    ...
    if st.session_state.show_splash:
        ...
        if movies is None:
            ...
            # Mobile Bottom Navigation Bar + viewport meta for mobile
            current_page = st.session_state.page
            st.markdown(f'''
            <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover">
            <div class="mobile-nav">
                <a href="?page=home&user={st.session_state.username}" class="nav-item {'active' if current_page=='home' else ''}" aria-label="Home">
                    <span class="nav-icon">🏠</span>Home
                </a>
                <a href="?page=recent&user={st.session_state.username}" class="nav-item {'active' if current_page=='recent' else ''}" aria-label="Recent">
                    <span class="nav-icon">🕐</span>Recent
                </a>
                <a href="?page=watchlist&user={st.session_state.username}" class="nav-item {'active' if current_page=='watchlist' else ''}" aria-label="Saved">
                    <span class="nav-icon">❤️</span>Saved
                </a>
                <a href="?page=liked&user={st.session_state.username}" class="nav-item {'active' if current_page=='liked' else ''}" aria-label="Liked">
                    <span class="nav-icon">👍</span>Liked
                </a>
            </div>
            <script>
                // Ensure all movie cards respond instantly to touch
                document.querySelectorAll('.movie-card, .nav-item, .share-icon').forEach(el => {{
